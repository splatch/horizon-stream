...
package org.opennms.horizon.shared.flows.values;
...
import org.opennms.horizon.shared.flows.exceptions.MissingTemplateException;
import org.opennms.horizon.shared.flows.ipfix.proto.DataRecord;
import org.opennms.horizon.shared.flows.ipfix.proto.FieldSpecifier;
import org.opennms.horizon.shared.flows.netflow9.proto.FlowSetHeader;
import org.opennms.horizon.shared.flows.session.Field;
import org.opennms.horizon.shared.flows.session.Session;
import org.opennms.horizon.shared.flows.session.Template;
    ...
    */
    public static InformationElement parserWithSubTemplateList(final String name, final Optional<Semantics> semantics) {
        return new InformationElement() {
            ...
            public Value<?> parse(final Session.Resolver resolver, final ByteBuf buffer) throws InvalidPacketException, MissingTemplateException {
                final Semantic semantic = Semantic.parse(buffer, BufferUtils.uint8(buffer));
                final int templateId = BufferUtils.uint16(buffer);
        ...
        return new InformationElement() {
            ...
            public Value<?> parse(final Session.Resolver resolver, final ByteBuf buffer) throws InvalidPacketException, MissingTemplateException {
                ...
                while (buffer.isReadable()) {
                    ...
                    final ByteBuf payloadBuffer = BufferUtils.slice(buffer, header.length - FlowSetHeader.SIZE);
