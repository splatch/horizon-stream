Feature: Alert Service Basic Functionality
  Background: Configure base URLs
    ...
    And Trigger events data
      ...
      | SNMP_Link_Down | 1     | 0        | MINUTE        | MAJOR    |                |
      | SNMP_Link_Up   | 1     | 0        | MINUTE        | CLEAR    | SNMP_Link_Down |
  ...
  Scenario: Verify when an event is received from Kafka, a new alert is created
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantA" with node 10 with severity "MINOR"
  ...
  Scenario: Verify that alerts can be cleared by other events
    ...
    Then List alerts for tenant "tenantA", with timeout 5000ms, until JSON response matches the following JSON path expressions
      ...
      | alerts[0].counter == 1      |
      | alerts[0].severity == MINOR |
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Up" with tenant "tenantA" with node 10 with severity "MINOR"
    Then List alerts for tenant "tenantA", with timeout 5000ms, until JSON response matches the following JSON path expressions
      | alerts.size() == 1            |
      | alerts[0].counter == 2        |
  ...
  Scenario: Verify alert reduction for duplicate events
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantF" with node 10 with severity "MINOR"
  ...
  Scenario: Verify page size 1 should return only 1 alert
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantG" with node 10 with severity "MINOR"
    Then List alerts for tenant "tenantG" with page size 1, with timeout 5000ms, until JSON response matches the following JSON path expressions
      ...
      | alerts[0].counter == 1 |
      | alerts[0].severity == MINOR |
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantG" with node 11 with severity "MINOR"
    Then List alerts for tenant "tenantG" with page size 1, with timeout 5000ms, until JSON response matches the following JSON path expressions
      | alerts.size() == 1          |
      | alerts[0].counter == 1      |
      | alerts[0].severity == MINOR |
  ...
  Scenario: Verify find alert sorted by id
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantH" with node 10 with severity "MINOR"
    Then List alerts for tenant "tenantH", with timeout 10000ms, until JSON response matches the following JSON path expressions
      | alerts.size() == 1 |
      | alerts[0].counter == 1 |
      | alerts[0].severity == MINOR |
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantH" with node 11 with severity "MAJOR"
  ...
  Scenario: Verify find alert filtered by severity MAJOR and MINOR
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantI" with node 10 with severity "MINOR"
    Then List alerts for tenant "tenantI", with timeout 5000ms, until JSON response matches the following JSON path expressions
      | alerts.size() == 1 |
      | alerts[0].counter == 1 |
      | alerts[0].severity == MINOR |
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantI" with node 11 with severity "MAJOR"
    ...
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantI" with node 12 with severity "CRITICAL"
  ...
  Scenario: Verify find alert filtered by time
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantJ" with node 10 with severity "MINOR" with produced time 23h ago
    Then List alerts for tenant "tenantJ", with timeout 5000ms, until JSON response matches the following JSON path expressions
      | alerts.size() == 1 |
      | alerts[0].counter == 1 |
      | alerts[0].severity == MINOR |
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantJ" with node 11 with severity "MAJOR"
    Then List alerts for tenant "tenantJ", with timeout 5000ms, until JSON response matches the following JSON path expressions
      | alerts.size() == 2          |
      | alerts[0].counter == 1      |
      | alerts[0].severity == MINOR |
      | alerts[1].severity == MAJOR |
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantJ" with node 12 with severity "MINOR" with produced time 8 days ago
    Then List alerts for tenant "tenantJ", with timeout 5000ms, until JSON response matches the following JSON path expressions
      | alerts.size() == 2              |
      | alerts[0].counter == 1          |
      | alerts[0].severity == MINOR     |
      | alerts[1].severity == MAJOR     |
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantJ" with node 13 with severity "MINOR" with produced time last month
    ...
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantK" with node 10 with severity "MINOR"
    Then List alerts for tenant "tenantK", with timeout 5000ms, until JSON response matches the following JSON path expressions
      | alerts.size() == 1 |
      | alerts[0].counter == 1 |
      | alerts[0].severity == MINOR |
    Then Send event with UEI "uei.opennms.org/generic/traps/SNMP_Link_Down" with tenant "tenantK" with node 11 with severity "MAJOR"
    Then List alerts for tenant "tenantK", with timeout 5000ms, until JSON response matches the following JSON path expressions
      | alerts.size() == 2          |
      | alerts[0].counter == 1      |
      | alerts[0].severity == MINOR |
