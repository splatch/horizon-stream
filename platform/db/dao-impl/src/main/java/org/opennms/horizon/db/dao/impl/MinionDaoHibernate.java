...
public class MinionDaoHibernate  extends AbstractDaoHibernate<OnmsMinion, String>  implements MinionDao {
    public MinionDaoHibernate(EntityManagerHolder entityManagerHolder) {
        super(entityManagerHolder, OnmsMinion.class);
    }

    @Override
    public OnmsMinion findById(final String id) {
        TypedQuery<OnmsMinion> query = getEntityManager().createQuery("SELECT minion from OnmsMinion as minion where minion.id = :id", OnmsMinion.class);
