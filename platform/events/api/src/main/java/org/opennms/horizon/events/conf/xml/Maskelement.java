...
import static org.opennms.horizon.events.conf.xml.EventMatchers.field;
import static org.opennms.horizon.events.conf.xml.EventMatchers.valueEqualsMatcher;
import static org.opennms.horizon.events.conf.xml.EventMatchers.valueMatchesRegexMatcher;
import static org.opennms.horizon.events.conf.xml.EventMatchers.valueStartsWithMatcher;
...
public class Maskelement implements Serializable {
    ...
    public org.opennms.horizon.events.conf.xml.EventMatcher constructMatcher() {
        List<org.opennms.horizon.events.conf.xml.EventMatcher> valueMatchers = new ArrayList<org.opennms.horizon.events.conf.xml.EventMatcher>(m_values.size());
        ...
        if (valueMatchers.size() == 1) {
            ...
            org.opennms.horizon.events.conf.xml.EventMatcher[] matchers = valueMatchers.toArray(new org.opennms.horizon.events.conf.xml.EventMatcher[valueMatchers.size()]);
            return org.opennms.horizon.events.conf.xml.EventMatchers.or(matchers);
        }
