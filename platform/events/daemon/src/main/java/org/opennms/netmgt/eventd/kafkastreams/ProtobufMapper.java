...
//        }
    ...
    public Event toEvent(OpennmsModelProtos.Event event) {
        if (event == null) {
            return null;
        }
        Event result = new Event();
        result.setDbid((int) event.getId());
        result.setUei(event.getUei());
        result.setSource(event.getSource());
        result.setSeverity(event.getSeverity().toString());
        result.setDescr(event.getDescription());
        result.setDistPoller(event.getDistPoller());
        if (!event.getLogMessage().isEmpty()) {
            Logmsg logmsg = new Logmsg();
            logmsg.setContent(event.getLogMessage());
        }
...
//        }
        ...
        result.setInterface(event.getIpAddress());

        for (OpennmsModelProtos.EventParameter parm : event.getParameterList()) {
//            if (parm.getName() == null || parm.getValue() == null) {
//                continue;
//            }
            result.addParm(new Parm(parm.getName(), parm.getValue()));
        }

        result.setTime(new Date(event.getTime()));
        result.setCreationTime(new Date(event.getCreateTime()));
        result.setSnmp(buildSnmpInfo(event.getSnmpInfo()));
    ...
    private Snmp buildSnmpInfo(OpennmsModelProtos.SnmpInfo snmp) {
        Snmp result = new Snmp();

        result.setId(snmp.getId());
        result.setVersion(snmp.getVersion());
        result.setSpecific(snmp.getSpecific());
        result.setGeneric(snmp.getGeneric());
        result.setCommunity(snmp.getCommunity());
        result.setTrapOID(snmp.getTrapOid());
...
//            }
    ...
    public OpennmsModelProtos.AlarmFeedback.Builder toAlarmFeedback(OpennmsModelProtos.AlarmFeedback alarmFeedback) {
