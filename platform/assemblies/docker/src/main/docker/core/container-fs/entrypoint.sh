...
umask 002
MINION_HOME="/opt/horizon-stream"
MINION_CONFIG="/opt/horizon-stream/etc/org.opennms.minion.controller.cfg"
MINION_PROCESS_ENV_CFG="/opt/horizon-stream/etc/minion-process.env"
MINION_SERVER_CERTS_CFG="/opt/horizon-stream/etc/minion-server-certs.env"
MINION_OVERLAY_ETC="/opt/horizon-stream-etc-overlay"
CONFD_KEY_STORE="/opt/horizon-stream/minion-config.yaml"
CONFD_CONFIG_DIR="/opt/horizon-stream/confd"
CONFD_BIN="/usr/local/bin/confd"
CONFD_CONFIG_FILE="${CONFD_CONFIG_DIR}/confd.toml"
CACERTS="/opt/horizon-stream/cacerts"
...
configure() {
  ...
  if [[ -f "$MINION_SERVER_CERTS_CFG" ]]; then
    ...
    while read certid; do
      [[ $certid =~ ^#.* ]] && continue
      keytool -importcert -file "/opt/horizon-stream/server-certs/$certid" -alias "$certid" -keystore "$CACERTS" -storepass changeit -noprompt
