...
package org.opennms.horizon.core.lib.timeout;
...
public class TimeoutTracker {
    ...
    public TimeoutTracker(Map<String,?> parameters, int defaultRetry, int defaultTimeout) {
        m_retry = ParameterMapUtil.getKeyedInteger(parameters, "retry", defaultRetry);

        // make sure the timeout is a least 10 millis
        m_timeoutInMillis = Math.max(10L, ParameterMapUtil.getKeyedInteger(parameters, "timeout", defaultTimeout));
        ...
        m_strictTimeouts = ParameterMapUtil.getKeyedBoolean(parameters, "strict-timeout", false);
