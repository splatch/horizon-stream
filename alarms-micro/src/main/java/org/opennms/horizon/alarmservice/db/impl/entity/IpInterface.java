...
public class IpInterface extends Entity implements Serializable {
    ...
    @Embedded
    @Column(length=1)
    @Type(type= "org.opennms.horizon.alarmservice.db.impl.utils.CharacterUserType.CharacterUserType")
    ...
    @Transient
    private List<MetaData> requisitionedMetaData = new ArrayList<>();

    public IpInterface(String ipAddr) {
        ipAddress = InetAddressUtils.getInetAddress(ipAddr);
    }

    public IpInterface(InetAddress ipAddr) {
        ipAddress = ipAddr;
    }
    ...
    public void addRequisionedMetaData(final MetaData onmsMetaData) {
    ...
    public void addMetaData(final String context, final String key, final String value) {
        ...
        final Optional<MetaData> entry = getMetaData().stream()
        ...
        if (entry.isPresent()) {
            ...
            getMetaData().add(new MetaData(context, key, value));
        }
    }
    ...
    public void removeMetaData(final String context, final String key) {
        Objects.requireNonNull(context);
        Objects.requireNonNull(key);
        final Iterator<MetaData> iterator = getMetaData().iterator();

        while (iterator.hasNext()) {
            final MetaData onmsNodeMetaData = iterator.next();
     ...
     *
     * @param scannedIface a {@link IpInterface} object.
     */
    public void mergeInterfaceAttributes(IpInterface scannedIface) {
...
//    }
    ...
    public void mergeMetaData(IpInterface scanned) {
     ...
     * @param scannedIface a {@link IpInterface} object.
     */
    public void updateSnmpInterface(IpInterface scannedIface) {
