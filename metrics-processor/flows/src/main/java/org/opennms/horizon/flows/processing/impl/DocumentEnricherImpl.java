...
import org.opennms.horizon.flows.processing.cache.Cache;
import org.opennms.horizon.flows.processing.cache.CacheBuilder;
import org.opennms.horizon.flows.processing.cache.CacheConfig;
import org.opennms.horizon.flows.processing.cache.CacheConfigBuilder;
...
public class DocumentEnricherImpl {
    ...
    public List<EnrichedFlow> enrich(final Collection<Flow> flows, final FlowSource source) {
        ...
        return sessionUtils.withTransaction(() -> flows.stream().flatMap(flow -> {
            ...
            // Locality
            if (flow.getSrcAddr() != null) {
                document.setSrcLocality(isPrivateAddress(flow.getSrcAddr()) ? Locality.PRIVATE : Locality.PUBLIC);
            }
            if (flow.getDstAddr() != null) {
                document.setDstLocality(isPrivateAddress(flow.getDstAddr()) ? Locality.PRIVATE : Locality.PUBLIC);
            }
    ...
    private Optional<NodeInfo> getNodeInfo(final InterfaceToNodeCache.Entry entry) {
        ...
        try (Timer.Context ctx = this.nodeLoadTimer.time()) {
            onmsNode = this.nodeDao.get(String.valueOf(entry.nodeId));
        }
