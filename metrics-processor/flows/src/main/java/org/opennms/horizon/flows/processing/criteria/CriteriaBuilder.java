...
package org.opennms.horizon.flows.processing.criteria;
...
import org.opennms.horizon.flows.processing.criteria.restrictions.AllRestriction;
import org.opennms.horizon.flows.processing.criteria.restrictions.Restriction;
import org.opennms.horizon.flows.processing.criteria.restrictions.Restrictions;
import org.opennms.horizon.flows.processing.criteria.restrictions.SqlRestriction;
...
public class CriteriaBuilder {
    ...
    public CriteriaBuilder fetch(final String attribute, final Fetch.FetchType type) {
        m_fetch.add(new Fetch(attribute, type));
        return this;
    }

    public CriteriaBuilder join(final String associationPath, final String alias) {
        return alias(associationPath, alias, Alias.JoinType.LEFT_JOIN);
    }
    ...
    public CriteriaBuilder join(final String associationPath, final String alias, final Alias.JoinType type) {
        return alias(associationPath, alias, type);
    }

    public CriteriaBuilder alias(final String associationPath, final String alias, final Alias.JoinType type) {
        return alias(associationPath, alias, type, null);
    }

    public CriteriaBuilder alias(final String associationPath, final String alias, final Alias.JoinType type, final Restriction joinCondition) {
    ...
    public CriteriaBuilder sql(final String sql, final Object parameter, final SqlRestriction.Type type) {
        addRestriction(Restrictions.sql((String) sql, parameter, type));
        return this;
    }

    public CriteriaBuilder sql(final String sql, final Object[] parameters, final SqlRestriction.Type[] types) {
