Feature: Inventory Processing
  ...
  Scenario: Deletion of a device causes Task Definitions Removals to be Requested
    ...
    Then verify the task set update is published with removal of task with suffix "icmp-detector" within 30000ms
    Then verify the task set update is published with removal of task with suffix "snmp-detector" within 30000ms
