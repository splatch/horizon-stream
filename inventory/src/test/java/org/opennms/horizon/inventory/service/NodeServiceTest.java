...
public class NodeServiceTest {
    ...
    public void createNodeWithLocationDefaultLocationExist() {
        ...
        doReturn(Optional.of(location)).when(monitoringLocationRepository).findByLocationAndTenantId(GlobalConstants.DEFAULT_LOCATION, tenantID);
        nodeService.createNode(nodeCreate, tenantID);
        verify(monitoringLocationRepository).findByLocationAndTenantId(GlobalConstants.DEFAULT_LOCATION, tenantID);
    ...
    public void createNodeWithLocationDefaultLocationNotExist() {
        NodeCreateDTO nodeCreate = NodeCreateDTO.newBuilder()
            .setLabel("test-node")
            .setManagementIp("127.0.0.1").build();
        doReturn(Optional.empty()).when(monitoringLocationRepository).findByLocationAndTenantId(GlobalConstants.DEFAULT_LOCATION, tenantID);
        ...
        assertThat(captor.getValue().getLocation()).isEqualTo(GlobalConstants.DEFAULT_LOCATION);
        verify(nodeRepository).save(any(Node.class));
        verify(ipInterfaceRepository).save(any(IpInterface.class));
    }
}
